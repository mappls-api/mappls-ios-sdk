...
Mappls's Map SDK for iOS lets you easily add Mappls Map and web services to your own iOS app. Mappls's Map SDK for iOS supports iOS SDK 13.0 and above and Xcode 16 or later. You can have a look at the map and features you will get in your own app by using the Mappls's Maps app for iOS. The SDK handles map tiles download and their display along with a bunch of controls and native gestures.
...
| :------ | :---- | :---------- |
| `6.1.4` | 02 Apr 2026 | - Fixed issue with PointFeature  mapplsPin |
| `6.1.3` | 01 Apr 2026 | - Added 3D building plugin to hide and show extrusion on MapView <br> - Fixed a issue for carplay full screen map <br>- Bug fixes and improvements. <br> - Added support of style editor custom styles. |
...
### [By Interface Builder](#By-Interface-Builder)
...
### [By Programmatically](#By-Programmatically)
...
mapView.zoomLevel = 4
//or
mapView.setZoomLevel(19, animated: false)
```
...
### [Custom Marker (Change Default Marker Icon)](#custom-marker)
...
### [Show Info Window (Callout)](#show-info-window)
...
### [3D Building plugin](#3d-building-plugin)

`BuildingMapPlugin` is class responsiable for hide and show the 3d builing on map. 
this class can be initilize after MapplsMapView initilization.

  #### Usage overview:
  1. Initialize with a `MapplsMapView` after creating/assigning the map view.
  2. Call `updateBuildingLayers:` after the style loads (and whenever it changes) to capture original expressions.
  3. Use `show3DBuildingsWithDuration:` / `hide3DBuildingsWithDuration:` to animate global visibility.
  4. Optionally call `hide3DBuildingsAtCoordiante:` to flatten the building under a coordinate.
...
### Methods
  ...
  **initWithMapView**
  ...
  Initializes the plugin with the given map view. <br>
  Parameter mapView: The `MapplsMapView` whose style and features will be managed.

```swift
let plugin = BuildingMapPlugin(mapView: self.mapView)
```

**updateBuildingLayers**

Scans the provided style to discover all fill extrusion layers and caches their original height/base expressions. Call this after the style has finished loading and whenever the style changes. <br>

Parameter style: The active `MGLStyle` to scan.

```swift
plugin.updateBuildingLayers(style)
```

**hide3DBuildingsWithDuration**

Animates to hide all 3D buildings by scaling extrusion heights/bases to zero.

 Parameter duration: Duration in seconds. Use 0 to apply immediately.

```swift
plugin.hide3DBuildings(withDuration: 0.5)
```

**show3DBuildingsWithDuration**

Animates to show all 3D buildings by restoring extrusion heights/bases to their original values.

Parameter duration: Duration in seconds. Use 0 to apply immediately

```swift
plugin.show3DBuildings(withDuration: 0.5)
```

**hide3DBuildingsAtCoordiante**

Flattens (hides) the 3D building at the specified coordinate, if any; restores if none is found.

Parameter coordinate: The map coordinate to query for a building feature (by `UID`).

```swift
plugin.hide3DBuildings(atCoordiante: CLLocationCoordinate2D(latitude: 28.904188, longitude: 77.32))
```
...
```swift
class BuildingPluginExample: UIViewController, MapplsMapViewDelegate {  
    var plugin: BuildingMapPlugin?
    var mapView: MapplsMapView!
    override func viewDidLoad() {
        super.viewDidLoad()
        self.mapView = MapplsMapView(frame: self.view.bounds)
        mapView.delegate = self
        view.addSubview(mapView)
        
        plugin = BuildingMapPlugin(mapView: mapView)
    }
