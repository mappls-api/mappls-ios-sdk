...
To initialise the MapplsAPICore SDK and begin the authentication process, we need to call the `initilizeSDK` function. This function takes two optional parameters: `configPath` and `olfPath`, both of type `URL`. These parameters specify the paths to the config and olf files. 

```swift
MapplsAPICoreManager().initilizeSDK(configPath: configPath, olfPath: olfPath)
```
