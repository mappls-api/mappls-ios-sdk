...
| :---- | :---- | :---- |
| `3.0.4`| 31 March 2026 | - Removed hyper local parameter from MapplsAutoSuggest API <br>- Geocode api Reqest `searchType` is added <br>- `SkipWaypoints`, `tollPass`, `eType` is added in `RouteOptions` class |
...
Mappls Map SDK for iOS lets you easily add Mappls Map and services to your own iOS app.It supports iOS SDK 13.0 and above and Xcode 16 or later. You can have a look at the map and features you will get in your own app by using the Mappls Map app for iOS. The SDK handles Map Display along with a bunch of controls and native gestures.
...
Class used for Autosuggest search is `MapplsAutoSuggestManager`.
...
// or
MapplsAutoSuggestManager *autoSuggestManager = [[MapplsAutoSuggestManager alloc] init];
...
Class used for geocode is `MapplsReverseGeocodeManager`.
...
Class used for nearby search is `MapplsNearByManager`.
...
//Or
MapplsNearByManager * nearByManager = [[MapplsNearByManager alloc] init];
...
Class used for Mappls Pin search is `MapplsPlaceDetailManager`
...
//or
MapplsPlaceDetailManager * placeDetailManager = [[MapplsPlaceDetailManager alloc] init];
...
Class used for geocode is `MapplsAtlasGeocodeManager`
...
// or
MapplsAtlasGeocodeManager * atlasGeocodeManager = [[MapplsAtlasGeocodeManager alloc] init];
...
// or
let atlasGeocodeManager = MapplsAtlasGeocodeManager()
...
Use `Directions` to get route between locations.
...
### [Cancel Directions Request](Cancel-Directions-Request)

To cancel the directions request, you need to retain the task return by the `calculate` function, cancel that task and you also need set `cancelDirectionsRequest` boolean value to `true` 

```swift
let directionsManager = Directions()
let directionsTask = directionsManager.calculate(options, completionHandler: handler)
directionsTask?.cancel()
directionsManager.cancelDirectionsRequest = true
```


### [Get ManeuverId](Get-ManeuverId)

In the `MapplsDirectionsUtility` class, the `getShortInstructionAndManeuverId` function returns a tuple containing the maneuverId and short instructions.

```swift
let maneuverID = directionUtility.getShortInstructionAndManeuverId(step: routeStep).maneuverID
let shortInstruction = directionUtility.getShortInstructionAndManeuverId(step: routeStep).instruction
```
...
Class used for driving distance is `MapplsDrivingDistanceMatrixManager`.
...
Class used to get list of POI along a route is `MapplsPOIAlongTheRouteManager`.
...
- **partnersFlag**  This object provides information about the partner. For this, the user needs to provision the 'partnerId or showPartnerInfo' claim.
- **hourOfOperation** To allow access open_close_hours.
...
poiAlongTheRouteManager.getPOIsAlongTheRoute(poiAlongTheRouteOptions) { (result, error) in
	if let error = error {
...
poiAlongTheRouteOptions.buffer = 300;
[poiAlongTheRouteManager getPOIsAlongTheRoutekWithOptions:poiAlongTheRouteOptions completionHandler:^(MapplsPOIAlongTheRouteResult * _Nullable result, NSError * _Nullable error) {
    if (error) {
            
    } else if (result) {
        NSArray<MapplsPOISuggestion*> * suggestions = result.suggestions;
...
Class used to get list of Nearby Reports API is `MapplsNearbyReportManager`
...
Class used to get list of Current Weather Condition API is `MapplsWeatherManager`.
