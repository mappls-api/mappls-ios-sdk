...
class DrivingRangeSettingViewController: UITableViewController {
    ...
    @objc func doneButtonClicked() {
        if sourceLocations.indices.contains(1) && self.sourceLocations.last != "Enter time in min." && self.sourceLocations.last != ""  && userLocation != nil {
            var drivingRangeRangeTypeInfo : MapplsDrivingRangeRangeTypeInfo!
            var drivingRangeOptimalSpeed: MapplsDrivingRangeSpeed!
            if let rangeInfos = sourceLocations.last {
                var contours = [MapplsDrivingRangeContour]()
//                if rangeInfos.contains(",") {
                    let rageInfo = rangeInfos.split(separator: ",")
                    for r in rageInfo {
                        if let intValue = Int(r) {
                            contours.append(MapplsDrivingRangeContour(value: intValue))
                        }
                    }
//                }
                if rangeInfo == 0 {
                    drivingRangeRangeTypeInfo = MapplsDrivingRangeRangeTypeInfo(rangeType: .time, contours: contours)
                } else {
                    drivingRangeRangeTypeInfo = MapplsDrivingRangeRangeTypeInfo(rangeType: .distance, contours: contours)
                }
            }
            
            if self.destinationLocations.indices.contains(1),let speedInfos = self.destinationLocations.last  {
                if speedInfo == 0 {
                    drivingRangeOptimalSpeed = MapplsDrivingRangeOptimalSpeed()
                } else if speedInfo == 1 {
                    drivingRangeOptimalSpeed = MapplsDrivingRangePredictiveSpeedFromCurrentTime()
                } else if speedInfo == 2 {
                    if  speedInfos != "", let timeStamp = TimeInterval(speedInfos) {
                        drivingRangeOptimalSpeed = MapplsDrivingRangePredictiveSpeedFromCustomTime(timestamp: timeStamp)
                    }
                    
                }
            }
            let location = CLLocation(latitude: userLocation.latitude, longitude: userLocation.longitude)
            var drivingRangeOptions: MapplsDrivingRangeOptions!
            guard drivingRangeRangeTypeInfo != nil else {
                emptyFieldError()
                return
            }
            if let option = drivingRangeOptimalSpeed {
                drivingRangeOptions  = MapplsDrivingRangeOptions(location: location, rangeTypeInfo: drivingRangeRangeTypeInfo, speedTypeInfo: option)
            } else {
                drivingRangeOptions  = MapplsDrivingRangeOptions(location: location, rangeTypeInfo: drivingRangeRangeTypeInfo)
            }
...
extension DrivingRangeSettingViewController: MapplsAutocompleteViewControllerDelegate {
