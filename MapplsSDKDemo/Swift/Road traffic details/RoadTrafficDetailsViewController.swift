...
//        mapView.setMapCenterAtMapplsPin("mmi000", animated: true, completionHandler: nil)
//        mapView.zoomLevel = 16
    }
    ...
    func getRoadTrafficDetailsAPI(lat: Double, long: Double){
        tableDict.removeAll()
        let options = MapplsRoadTrafficDetailsOptions(latitude: lat, longitude: long)
        let manager = MapplsRoadTrafficDetailsManager.shared
        manager.getTrafficRoadDetailsResults(options) { resposnse, error in
            if let error = error {
                ...
                if let response  = resposnse {
                    let res: MapplsRoadTrafficDetailsResponse = response
                    if let result : MapplsRoadTrafficDetailsResult = res.result {
                        do {
