...
class AutoSuggestVC: UIViewController {
    ...
    private func getAutoSuggestResult(searchText: String , isTextSearch:Bool = false) {
        if isTextSearch == true {
            let textSearchManager = MapplsAtlasTextSearchManager.shared
            let textSearchOptions = MapplsTextSearchAtlasOptions(query: searchText)
            textSearchOptions.refLocation = CLLocation(latitude: 26.901190, longitude: 75.790539)
            textSearchManager.getTextSearchResult(textSearchOptions) { (result, error) in
            ...
            if !searchText.isEmpty && searchText.count > 2 {
                ...
                MapplsAutoSuggestManager.shared.getAutoSuggestionResults(autoSearchOptions) { result, error in
                    DispatchQueue.main.async {
                        ...
                        } else if let results = result?.suggestions, results.count > 0 {
