...
class LocationChooserTableViewDirectionUIPlugin: UITableViewController {
    ...
    var sourceLocations = [MapplsDirectionLocation]()
    var destinationLocations = [MapplsDirectionLocation]()
    var viaLocations = [MapplsDirectionLocation]()
    ...
    var selectedProfileDistance: MapplsDirectionsProfileIdentifier = .driving
    
    var selectedResourceDirection: MapplsDirectionsResourceIdentifier = .routeAdv
    var selectedProfileDirection: MapplsDirectionsProfileIdentifier = .driving
    ...
    override func viewDidLoad() {
        ...
        sourceLocations = [MapplsDirectionLocation(location: "", displayName: "", displayAddress: "", locationType: .none)]
        destinationLocations = [MapplsDirectionLocation(location: "", displayName: "", displayAddress: "", locationType: .none)]
    }
    ...
    @objc func addLocationButtonPressed(_ button: UIButton) {
        ...
        if emptySourceFields.count > 0 || emptyDestinationFields.count > 0
            ...
            case sectionForSourceLocations:
                ...
                sourceLocations.append(MapplsDirectionLocation(location: "", displayName: "", displayAddress: "", locationType: .none))
                self.tableView.reloadData()
            case sectionForDestinationLocations:
                if destinationLocations.count >= 1 {
                    break
                }
                destinationLocations.append(MapplsDirectionLocation(location: "", displayName: "", displayAddress: "", locationType: .none))
                self.tableView.reloadData()
            case sectionForViaLocations:
                viaLocations.append(MapplsDirectionLocation(location: "", displayName: "", displayAddress: "", locationType: .none))
                self.tableView.reloadData()
            default:
                break
            }
        ...
        if let indexPath = indexPathForSourceOrDesitnation {
            if let latitude = selectedPlace.latitude, let longitude = selectedPlace.longitude {
                if indexPath.section == sectionForSourceLocations {
                    sourceLocations[indexPath.row] = MapplsDirectionLocation(location: "\(longitude),\(latitude)", displayName: selectedPlace.placeName, displayAddress: selectedPlace.placeAddress, locationType: .suggestion)
//                        "\(longitude),\(latitude)"
                } else if indexPath.section == sectionForDestinationLocations {
                    destinationLocations[indexPath.row] = MapplsDirectionLocation(location: "\(longitude),\(latitude)", displayName: selectedPlace.placeName, displayAddress: selectedPlace.placeAddress, locationType: .suggestion)
                } else if indexPath.section == sectionForViaLocations {
                    viaLocations[indexPath.row] = MapplsDirectionLocation(location: "\(longitude),\(latitude)", displayName: selectedPlace.placeName, displayAddress: selectedPlace.placeAddress, locationType: .suggestion)
                }
                tableView.reloadData()
            }
        }
    ...
    @objc func setMapplsPinValueForSourceOrDestinationLocationField(selectedPlace: MapplsAtlasSuggestion) {
        if let indexPath = indexPathForSourceOrDesitnation {
            if let mapplsPin = selectedPlace.mapplsPin {
                if indexPath.section == sectionForSourceLocations {
                    sourceLocations[indexPath.row] = MapplsDirectionLocation(location: mapplsPin, displayName: selectedPlace.placeName, displayAddress: selectedPlace.placeAddress, locationType: .suggestion)
                } else if indexPath.section == sectionForDestinationLocations {
                    destinationLocations[indexPath.row] = MapplsDirectionLocation(location: mapplsPin, displayName: selectedPlace.placeName, displayAddress: selectedPlace.placeAddress, locationType: .suggestion)
                } else if indexPath.section == sectionForViaLocations {
                    viaLocations[indexPath.row] = MapplsDirectionLocation(location: mapplsPin, displayName: selectedPlace.placeName, displayAddress: selectedPlace.placeAddress, locationType: .suggestion)
                }
                tableView.reloadData()
            }
        }
...
extension LocationChooserTableViewDirectionUIPlugin: MapplsAutocompleteViewControllerDelegate {
