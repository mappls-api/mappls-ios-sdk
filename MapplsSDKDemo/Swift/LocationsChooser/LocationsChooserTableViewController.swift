...
protocol LocationsChooserTableViewControllerDelegate {
    func locationsPikcedForDistances(sourceLocations: [String], destinationLocations: [String], resource: MapplsDistanceMatrixResourceIdentifier, profile: MapplsDirectionsProfileIdentifier)
    
    func locationsPikcedForDirections(sourceLocation: String, destinationLocation: String, viaLocations: [String], resource: MapplsDirectionsResourceIdentifier, profile: MapplsDirectionsProfileIdentifier, attributions: MapplsAttributeOptions)
}
...
class LocationsChooserTableViewController: UITableViewController {
    ...
    var attributions : MapplsAttributeOptions = []
    ...
    @objc func setMapplsPinValueForSourceOrDestinationLocationField(selectedPlace: MapplsAtlasSuggestion) {
        if let indexPath = indexPathForSourceOrDesitnation {
            if let mapplsPin = selectedPlace.mapplsPin {
                if indexPath.section == sectionForSourceLocations {
                    sourceLocations[indexPath.row] = mapplsPin
                } else if indexPath.section == sectionForDestinationLocations {
                    destinationLocations[indexPath.row] = mapplsPin
                } else if indexPath.section == sectionForViaLocations {
                    viaLocations[indexPath.row] = mapplsPin
                }
                tableView.reloadData()
            }
        }
...
extension LocationsChooserTableViewController: MapplsAutocompleteViewControllerDelegate {
