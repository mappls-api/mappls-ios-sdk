//
//  CameraSubOptionsViewController.swift
//  MapplsSDKDemo
//
...
class CameraSubOptionsViewController: UIViewController {
    
    var navBarView: NavigationBarView!
    var optionSelected: OptionsEnum!
    var cameraSubOptionsView: UICollectionView!
    ...
    func setSubOptionsView() {
        ...
        cameraSubOptionsView = UICollectionView(frame: .zero, collectionViewLayout: layout)
        cameraSubOptionsView.showsVerticalScrollIndicator = false
        cameraSubOptionsView.delegate = self
        cameraSubOptionsView.register(SubOptionCollectionViewCell.self, forCellWithReuseIdentifier: SubOptionCollectionViewCell.identifier)
        cameraSubOptionsView.translatesAutoresizingMaskIntoConstraints = false
        view.addSubview(cameraSubOptionsView)
        
        NSLayoutConstraint.activate([
            cameraSubOptionsView.leadingAnchor.constraint(equalTo: view.safeAreaLayoutGuide.leadingAnchor, constant: 15),
            cameraSubOptionsView.trailingAnchor.constraint(equalTo: view.safeAreaLayoutGuide.trailingAnchor, constant: -15),
            cameraSubOptionsView.topAnchor.constraint(equalTo: searchView.bottomAnchor, constant: 10),
            cameraSubOptionsView.bottomAnchor.constraint(equalTo: view.safeAreaLayoutGuide.bottomAnchor)
        ])
    ...
    func makeDataSource() -> UICollectionViewDiffableDataSource<Int, SubOptionHashable> {
        let dataSource = UICollectionViewDiffableDataSource<Int, SubOptionHashable>(collectionView: cameraSubOptionsView) { collectionView, indexPath, itemIdentifier in
...
extension CameraSubOptionsViewController: CommonSearchBarDelegate {
...
extension CameraSubOptionsViewController: AppThemeChangeable {
    func appThemeChanged(theme: ThemeColors) {
        DispatchQueue.main.async {
            self.view.backgroundColor = theme.backgroundPrimary
            self.cameraSubOptionsView.backgroundColor = theme.backgroundPrimary
        }
    }
}

extension CameraSubOptionsViewController: UICollectionViewDelegate {
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        let data = optionSelected.subOption[indexPath.row]
        let vc = CameraViewController()
        vc.cameraSubOptionSelected = data
        navigationController?.pushViewController(vc, animated: true)
    }
}

extension CameraSubOptionsViewController: NavigationProtocol {
    func navigateBack() {
