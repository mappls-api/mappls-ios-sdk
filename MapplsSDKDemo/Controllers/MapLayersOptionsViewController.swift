//
//  MapLayersOptionsViewController.swift
//  MapplsSDKDemo
//
...
class MapLayersOptionsViewController: UIViewController {
    ...
    func setSubOptionsView() {
        let layout = UICollectionViewCompositionalLayout { section, layoutEnvironment in
            ...
            if layoutEnvironment.traitCollection.verticalSizeClass == .regular {
                if UIDevice.current.userInterfaceIdiom == .pad && (UIApplication.shared.statusBarOrientation == .landscapeLeft || UIApplication.shared.statusBarOrientation == .landscapeRight) {
    ...
    func openViewController<T: UIViewController>(vc: T) {
        navigationController?.pushViewController(vc, animated: true)
    }
}

extension MapLayersOptionsViewController: CommonSearchBarDelegate {
...
extension MapLayersOptionsViewController: UICollectionViewDelegate {
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        let data = optionSelected.subOption[indexPath.row]
        
        switch data {
        case .indoorMap:
            openViewController(vc: CustomIndoorViewController())
        case .drivingRangePlugin:
            openViewController(vc: DrivingRangePluginViewController())
        case .geoanalytics:
            openViewController(vc: GeoanalyticsViewController())
        default:
            let vc = MapLayersViewController()
            vc.mapLayersSubOptionSelected = data
            navigationController?.pushViewController(vc, animated: true)
        }
    }
}

extension MapLayersOptionsViewController: AppThemeChangeable {
...
extension MapLayersOptionsViewController: NavigationProtocol {
    func navigateBack() {
