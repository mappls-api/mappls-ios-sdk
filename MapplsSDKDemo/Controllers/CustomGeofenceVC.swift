...
class CustomGeofenceVC: UIViewController,UITextFieldDelegate,MapplsGeofenceViewDelegate,MapplsMapViewDelegate {
    ...
    func hasIntersectPoints(_ shape: MapplsGeofenceShape) {
        print(shape)
        dataGeofence1 = shape as MapplsGeofenceShape?
    }
    ...
    public var data: MapplsGeofenceShape?
    public var dataGeofence1: MapplsGeofenceShape?
    public var shapeEdit: Bool?
    public var editShapeIndex: Int?
    public var editGeofence: MapplsGeofenceShape?
    ...
    @IBAction func btnApply(_ sender: Any) {
        ...
        if shapeEdit != true {
            ...
            Task {
                await delegateShape?.shapeSend(geofenceShapeData.currentShapeArr,geofenView: geofenceView, nameString: geofenceShapeData.geofenceNameArray)
            }
        }
        else {
    ...
    @objc func rotate(sender: UIBarButtonItem) {
        let vctrl = UIStoryboard(name: "Main", bundle: nil).instantiateViewController(withIdentifier: "MapComponantsVC") as? MapComponentsVC
    ...
    func setUpUI() {
        navBarView = NavigationBarView(title: SubOptionsEnum.customGeofenceUI.rawValue)
        navBarView.addBottomShadow()
        navBarView.layer.zPosition = 5
        ...
        let leftButtonItem = UIBarButtonItem(image: UIImage(systemName: "chevron.left"), style: .plain, target: self, action: #selector(btnBack))
    ...
    func geofenceInitiaze() {
        let view: MapplsGeofenceView =  {
            geofenceView = MapplsGeofenceView.init(geofenceframe: .zero)
            return geofenceView
        }()
        view.delegate = self
        geofenceView.setMode(mode: .circle)
...
//        geofenceView.markerStrokeColor = UserDefaultsManager.markerStrokeColor
        ...
        self.view.insertSubview(geofenceView, belowSubview: bottomBackView)
        
        NSLayoutConstraint.activate([
            geofenceView.leadingAnchor.constraint(equalTo: self.view.leadingAnchor),
            geofenceView.topAnchor.constraint(equalTo: self.navBarView.bottomAnchor),
            geofenceView.trailingAnchor.constraint(equalTo: self.view.trailingAnchor),
            geofenceView.bottomAnchor.constraint(equalTo: self.view.safeAreaLayoutGuide.bottomAnchor)
        ])
...
extension CustomGeofenceVC: NavigationProtocol {
    func navigateBack() {
        navigationController?.popViewController(animated: true)
    }
}

extension CustomGeofenceVC: AppThemeChangeable {
    func appThemeChanged(theme: ThemeColors) {
        DispatchQueue.main.async {
            self.view.backgroundColor = theme.backgroundPrimary
            self.btnDrawing.setTitleColor(theme.textPrimary, for: .normal)
            self.bottomBackView.backgroundColor = theme.backgroundPrimary
            self.txtGeofenceName.textColor = theme.textPrimary
            self.txtGeofenceName.backgroundColor = theme.backgroundSecondary
            self.slider.minimumTrackTintColor = theme.accentBrandPrimary
            self.geofenceContainerView.backgroundColor = theme.backgroundPrimary
            self.btnDrawing.backgroundColor = theme.backgroundSecondary
            self.btnPolygonCreationMode.backgroundColor = theme.backgroundSecondary
            self.btnPolygonCreationMode.setTitleColor(theme.textPrimary, for: .normal)
            self.geofenceView.backgroundColor = theme.backgroundPrimary
        }
    }
}
