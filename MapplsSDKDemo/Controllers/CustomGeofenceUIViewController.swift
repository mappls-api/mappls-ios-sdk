//
//  CustomGeofenceUIViewController.swift
//  MapplsSDKDemo
//
...
struct geofenceShapeData {
    static var currentShapeArr = [MapplsGeofenceShape]()
    static var backClicked : Bool = false
    static var geofenceNameArray = [String]()
}

protocol shapeSendDataDelegate: AnyObject, Sendable {
    func shapeSend(_ shapeArr: [MapplsGeofenceShape], geofenView: MapplsGeofenceView, nameString: [String]) async
}

class CustomGeofenceUIViewController: UIViewController, shapeSendDataDelegate, MapplsMapViewDelegate {
    func shapeSend(_ shapeArr: [MapplsGeofenceShape], geofenView: MapplsGeofenceView, nameString: [String] ) {
    ...
    var geofenceShape = [MapplsGeofenceShape]()
    ...
    var shapeGeofenceView: MapplsGeofenceView?
    ...
    func drawCircle(circleCoordinate: CLLocationCoordinate2D, radius: Double) {
        let circleCoordinates1 = MapplsGeofenceView.getCircleCoordinate(centerCoordinate: circleCoordinate, radius: radius)
...
extension CustomGeofenceUIViewController: UITableViewDelegate,UITableViewDataSource {
    ...
    @objc func switchChanged(_ sender : UISwitch!){
        ...
        var selectedShapes = [MapplsGeofenceShape]()
            ...
            if let cell: ShapeCell = tblShape.cellForRow(at: IndexPath(row: index, section: 0)) as? ShapeCell {
                ...
                if cell.ShapeSwitch.isOn {
                    ...
                    if shape.type?.rawValue == "polygon" {
                        ...
                        let circleCoordinates1 = MapplsGeofenceView.getCircleCoordinate(centerCoordinate: shape.circleCenterCoordinate!, radius: shape.circleRadius ?? 0.0)
...
extension CustomGeofenceUIViewController: NavigationProtocol {
    func navigateBack() {
        navigationController?.popViewController(animated: true)
    }
}

extension CustomGeofenceUIViewController: AppThemeChangeable {
    func appThemeChanged(theme: ThemeColors) {
        DispatchQueue.main.async {
            self.view.backgroundColor = theme.backgroundPrimary
            self.tblShape.backgroundColor = theme.backgroundPrimary
        }
    }
}
