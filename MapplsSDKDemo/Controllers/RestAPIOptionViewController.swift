//
//  RestAPIOptionViewController.swift
//  MapplsSDKDemo
//
...
class RestAPIOptionViewController: UIViewController {
...
extension RestAPIOptionViewController: CommonSearchBarDelegate {
...
extension RestAPIOptionViewController: UICollectionViewDelegate {
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        let data = optionSelected.subOption[indexPath.row]
        if data == .autosuggestAPI {
            let vc = RestAPIAutosearchViewController()
            navigationController?.pushViewController(vc, animated: true)
            return
        } else if data == .tripPlanAPI {
            let vc = RestAPITripPlanViewController()
            navigationController?.pushViewController(vc, animated: true)
            return
        }
        let vc = RestAPIViewController()
        vc.restAPISubOptionSelected = data
        navigationController?.pushViewController(vc, animated: true)
    }
}

extension RestAPIOptionViewController: AppThemeChangeable {
...
extension RestAPIOptionViewController: NavigationProtocol {
    func navigateBack() {
