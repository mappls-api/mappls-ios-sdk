...
-(void)setUpGUI {
    ...
    _listArr = @[ @"Zoom Level", @"Zoom Level With Animation", @"Center With Animation", @"Current Location",@"Tracking Mode", @"Add Marker", @"Add Multiple Markers With Bounds", @"Polyline" ,@"Polygons",
                  @"Autosuggest", @"Geocode", @"Reverse Geocoding", @"Nearby Search", @"Place Detail", @"Distance Matrix", @"Distance Matrix ETA", @"Route Advance", @"Route Advance ETA", @"Feedback", @"Animate Marker", @"GeoJson Multiple Shapes", @"Custom Marker", @"Interior Polygons", @"Covid Layers", @"COVID-19 Safety Status", @"Autosuggest Widget"
                  ];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    SWITCH (strType) {
        ...
        CASE (@"Autosuggest Widget") {
            AutosuggestWidgetExamplesLauncherVC *controller = [[AutosuggestWidgetExamplesLauncherVC alloc]  initWithNibName:nil bundle:nil];
            [self.navigationController pushViewController:controller animated:YES];
            break;
        }
        DEFAULT {
