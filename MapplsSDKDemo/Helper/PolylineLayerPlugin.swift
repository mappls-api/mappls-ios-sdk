...
open class PolylineLayerPlugin: NSObject {
    ...
    private var routeAlternateColor: UIColor = .defaultAlternateLine
    private var trafficUnknownColor: UIColor = .trafficUnknown
    private var trafficLowColor: UIColor = .trafficLow
    private var trafficModerateColor: UIColor = .trafficModerate
    private var trafficHeavyColor: UIColor = .trafficHeavy
    private var trafficSevereColor: UIColor = .trafficSevere
    
    // Assign marker images
    private var sourceMarkerImage: UIImage? = UIImage(named: "source-location-pin")
    private var destinationMarkerImage: UIImage? = UIImage(named: "destination-icon")
    private var waypointMarkerImage: UIImage? = UIImage(named: "viapoint-image")
    
    private var routes: [Route]?
    private var tapGestureDistanceThreshold: CGFloat = 50
    
    // Initialize the plugin with a MapplsMapView
    public init(mapView: MapplsMapView) {
        ...
        if let closestRoutes = routes(closeTo: point),
           ...
           let index = self.routes?.firstIndex(of: firstClosestRoute) {
            if let routeArray = self.routes {
                ...
                self.showRoutes(routes: routeArray, focusedRouteIndex: index)
            }
    ...
    public func showRoutes(routes: [Route], focusedRouteIndex: Int = 0, legIndex: Int = 0, edgeInset: UIEdgeInsets? = nil){
